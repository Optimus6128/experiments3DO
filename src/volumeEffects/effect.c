...
static void updateFire()
{
    int x,y,z;
    int i;
    const int i0 = TEX_SIZE_2D + TEX_WIDTH + 1;
    const int i1 = TEX_SIZE - i0;
...
static Mesh *initTorchMesh()
{
	...
	torchTex = initGenTexture(16, 16, 8, 1, TEXGEN_NOISE, NULL);
	setPal(0, 31, 48,40,32, 96,80,40, torchTex->pal);
...
void effectInit()
{
    ...
    for (i=0; i<DOTS_DEPTH; ++i) {
        ...
        dotCel[i]->ccb_PLUTPtr = (PLUTChunk*)dotTexPal;
...
static void renderMariaQuads(bool backToFront)
{
	...
	posXtrans = mouseState.med_HorizPosition;
	posYtrans = mouseState.med_VertPosition;
    ...
    for (z=0; z<DOTS_DEPTH; ++z) {
        pz = recZ[tvPtr->z + posZtrans];
        q[0].x = (((tvPtr->x + posXtrans) * pz) >> (FP_CORE - FP_PROJ)) + SCREEN_WIDTH / 2;
        q[0].y = (((tvPtr->y + posYtrans) * pz) >> (FP_CORE - FP_PROJ)) + SCREEN_HEIGHT / 2;
        tvPtr++;

        pz = recZ[tvPtr->z + posZtrans];
        q[1].x = (((tvPtr->x + posXtrans) * pz) >> (FP_CORE - FP_PROJ)) + SCREEN_WIDTH / 2;
        q[1].y = (((tvPtr->y + posYtrans) * pz) >> (FP_CORE - FP_PROJ)) + SCREEN_HEIGHT / 2;
        tvPtr++;

        pz = recZ[tvPtr->z + posZtrans];
        q[3].x = (((tvPtr->x + posXtrans) * pz) >> (FP_CORE - FP_PROJ)) + SCREEN_WIDTH / 2;
        q[3].y = (((tvPtr->y + posYtrans) * pz) >> (FP_CORE - FP_PROJ)) + SCREEN_HEIGHT / 2;
        tvPtr++;

        pz = recZ[tvPtr->z + posZtrans];
        q[2].x = (((tvPtr->x + posXtrans) * pz) >> (FP_CORE - FP_PROJ)) + SCREEN_WIDTH / 2;
        q[2].y = (((tvPtr->y + posYtrans) * pz) >> (FP_CORE - FP_PROJ)) + SCREEN_HEIGHT / 2;
        tvPtr++;
...
static void renderTorch()
{
    ...
    rotateVerticesHw(torchMesh);
    translateAndProjectVertices(torchMesh);

    prepareTransformedGeometryCELs(torchMesh);
    renderTransformedGeometryCELs(torchMesh);
}
...
static void updateFromInput()
{
	...
	if (mouseState.med_ButtonBits & MouseLeft) {
		--mouseTravelZ;
	}

	if (mouseState.med_ButtonBits & MouseRight) {
		++mouseTravelZ;
	}

	if (mouseState.med_ButtonBits & MouseMiddle) {
		advPlasma += 3;
		updateFx();
	}
}
